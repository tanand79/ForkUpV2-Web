...
/**
 * Pass 2 — claim a guest-launched campaign via email token.
 *
 * Purpose: Laptop→mobile ownership. User opens claim link, signs in/up as the
 * emailed address only, then we attach them to the nonprofit + set created_by.
 *
...
export function GuestCampaignClaim() {
  const { goTo, update } = useCampaign();
  const params = useSearchParams();
  const token =
    params.get("token")?.trim() ||
    (typeof window !== "undefined"
      ? sessionStorage.getItem("forkup-guest-claim-token")?.trim() || ""
      : "");
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void fetchGuestCampaignClaim(token)
      .then((data) => {
  ...
  const claimNow = async () => {
    ...
    setError(null);
    try {
      const result = await postGuestCampaignClaim(token);
      ...
      update({
        ...
        nonprofitProfile: {
          ...
          organizationName: result.campaignName,
          contactName: "",
    ...
    return (
      <main className="mx-auto max-w-md px-5 py-12 text-center">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          {info.campaignName} was already linked to an account. Sign in to manage it.
        </p>
        <button
          type="button"
          className="mt-6 rounded-full bg-primary px-6 py-3 text-sm font-semibold text-primary-foreground"
  ...
  if (info.expired) {
    return (
      <main className="mx-auto max-w-md px-5 py-12 text-center">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Ask ForkUp support or relaunch with a fresh email claim link.
        </p>
      </main>
    );
  }
  ...
  return (
    <main className="mx-auto max-w-md px-5 py-12">
      ...
      <p className="mt-2 text-sm text-muted-foreground">
        <span className="font-semibold text-foreground">{info.campaignName}</span>
        {" — "}
